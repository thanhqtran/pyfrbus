...
def get_maxlead(lexed_eqs: List[List[Tuple[str, Optional[Tuple[str, int]]]]]) -> int:
    maxlead = 0
    for eq in lexed_eqs:
        tokens = cast(List[Tuple[str, int]], unzip(eq)[1][0:-1])
        for token in tokens:
...
# Same, but for periods after the terminal
def dupe_vars_terminal(var_list: List[str], n_periods: int, maxlead: int) -> List[str]:
    return [
        f"{var}_{i}" for i in range(n_periods, n_periods + maxlead) for var in var_list
    ]
