...
class Frbus:
    def __init__(self, filepath: str, mce: Optional[str] = None):
        ...
        if mce:
            # Throw error if invalid mce type is given, or variables do not exist
            if (type(mce) != list and mce not in constants.CONST_MCE_TYPES) or (
                type(mce) == list
                and any([varname not in self.endo_names for varname in mce])
            ):
        ...
        # If model is forward looking, store the maximum lead length
        self.maxlead = equations.get_maxlead(self.lexed_eqs) if self.has_leads else 0
    ...
    def _solve_setup(
        ...
        if self.has_leads:
            data = _populate_mce_data(
                data, start, len(pd.period_range(start, end, freq="Q")) + self.maxlead
            )
        ...
        if (
            ...
            if self.has_leads:
                self._mce_setup(data, start, end)
    ...
    def _mce_setup(self, data: DataFrame, start: str, end: str):
        ...
        # Handle endo terminal condition as exos
        terminals = equations.dupe_vars_terminal(
            self.endo_names, n_periods, self.maxlead
        )

        # Duplicate exogenous variables as leads, including possible terminals
        dupe_exos = equations.dupe_vars(self.exo_names, n_periods + self.maxlead)

        self.endo_names += dupe_endos
        self.exo_names += dupe_exos + terminals
        ...
        self.lexed_eqs = lexing.remove_leads(self.lexed_eqs)
    ...
    def _reset_model(self) -> None:
        ...
        # Remove _trac for exogenized variables
        idxs = sorted(
            [self.exo_names.index(f"{x}_trac") for x in self.exoglist], reverse=True
        )
        [self.exo_names.pop(i) for i in idxs]
