...
# For mypy typing
from typing import List, Dict, cast, Tuple, Union
...
def mce_from_xml(
    root: Element, mce_type: Union[str, List]
    ...
    # If a list of variables was passed in, then return only those equations
    if type(mce_type) == list:
        tmp_zip = [x for x in zip(mce_eqs, mce_vars) if x[1] in mce_type]
        mce_eqs, mce_vars = ([x[0] for x in tmp_zip], [x[1] for x in tmp_zip])
...
def get_mce_xpath(mce_type: Union[str, List]) -> str:
    if mce_type == "all":
    ...
    elif type(mce_type) == list:
        # If a list is passed in, use only vars in the list
        return (
            "./variable["
            + reduce(
                lambda first, second: first + " or " + second,
                [f"name='{v}'" for v in mce_type],
            )
            + "]/mce_equation/python_equation"
        )
